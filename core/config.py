...
# 2026-09-08、ユーザー指示で「科目×教員の上限」を撤廃したため実質無制限にしている。
# 同一学生が同じ科目×教員へ複数投稿する重複投稿禁止（review_submit_api.py の dup_review）は
# この定数とは無関係に維持される。残り枠バッジ・「募集終了」（件数由来）の表示も、この値が
# 大きいことで常に空き扱いになり出なくなる（オンデマンド配信の募集終了は別ロジックで存続）。
MAX_REVIEWS_PER_COURSE_SECTION = 10 ** 9
...
# 2026-09-08、ユーザー指示でレビュー募集の締切扱いは撤廃した。現在この集合は
# 「科目閲覧LIFFに『他教員クラスも内容は同一』の注記を出す」「レビュー投稿フォームの
# 担当教員候補に『どちらを選んでもかまいません』と案内する」用途のみに使う。
# 投稿自体は担当教員を問わず通常どおり受け付ける。
...
# レビュー投稿フォームのコメント欄の最低文字数（strip後）。これ未満はフォームJS・
