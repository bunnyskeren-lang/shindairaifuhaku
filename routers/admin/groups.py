...
from core.groups import generate_group_code, group_payout_breakdown, group_stats
...
async def admin_groups(request: Request, _: str = Depends(check_admin)):
...
async def admin_group_create(
    name: str = Form(""), note: str = Form(""), _: str = Depends(check_admin),
):
    name = name.strip()[:100]
    if not name:
