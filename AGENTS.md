...
│       ├── groups.py                               ← /admin/groups*（団体の作成・名称変更・有効/無効・精算記録。団体別の発生額/精算済み/未精算残高。個人名・学籍番号は出さない）
...
| `inquiries` | お問い合わせ（質問・情報の誤りの指摘・新情報の追加提案・誤送信・誤字脱字の指摘等）。フォーム送信時にそのまま作成（メールアドレス認証なし）。`status`は'pending'/'handled'、`/admin/inquiries`で対応 |
