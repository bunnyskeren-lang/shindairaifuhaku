...
| `subject_unlocks` | レビュー閲覧権の解除記録（line_user_id, subject_id）。デフォルトでは他人のレビューは閲覧できず、会員登録（初回）で`REGISTRATION_WELCOME_UNLOCK_CREDITS`（1枚）、自分のレビューが1件承認されるたびに`REVIEW_APPROVAL_UNLOCK_CREDITS`（3枚）が`user_profiles.unlock_credits`に加算され、任意の科目でチケットを1枚消費して解除する（`routers/liff_api.py` `/api/course/{id}/unlock`）。語尾バリアントグループはグループ内の全subject_idをまとめて解除する |
