...
MANUAL_VARIANT_GROUPS: tuple[dict, ...] = (
    ...
    # 注意（2026-09-06発覚・修正）: 惑星学基礎(演習含む)は段階進行科目でⅠ〜Ⅴの担当教員が
    # 全員異なる（廣瀬仁/杉岡裕子/吉岡祥一/山崎和仁/山本由弦/清杉孝司等）。この教員不一致は
    # 上記の手動統合を決めた上で許容済みの前提であり、誤統合ではない。にもかかわらず同日実施
    # した「担当教員食い違いによる一括統合解除」（後述subjects.variant_merge_excluded一括設定
    # 作業）がこの前提を考慮せず両グループ11科目を巻き込んで誤ってvariant_merge_excluded=true
    # にしてしまい、この手動グループ自体が丸ごと非表示になる回帰を起こした（compute_variant_
    # display_groups()はグループ内に1件でもextra_excluded_namesがあるとグループ全体を
    # スキップするため）。今後、教員食い違いを理由にした一括統合解除（DB直接操作・管理画面
    # ボタンの一括実行いずれも）を行う際は、必ず_MANUAL_VARIANT_GROUP_NAMES（=MANUAL_VARIANT_
    # GROUPSに含まれる科目名）を対象から除外すること。
    ...
    {
        "names": ("近現代アート論A", "近現代アート論B"),
        "label": "近現代アート論(A/B)",
    },
    {
        "names": ("近現代モード論A", "近現代モード論B"),
        "label": "近現代モード論(A/B)",
    },
    {
        "names": ("近現代社会思想論A", "近現代社会思想論B"),
        "label": "近現代社会思想論(A/B)",
    },
    {
