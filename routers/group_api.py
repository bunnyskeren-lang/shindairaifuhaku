"""団体コードの照合API（レビュー投稿フォームの「団体コード」欄が入力時に呼ぶ。2026-09-26、core/groups.py）。"""
...
# 団体コードの総当たり（存在する団体名の探り出し）を防ぐため、IPあたり1分10回まで
...
async def group_lookup(request: Request, _rl: None = Depends(_lookup_rate_limit)):
    """団体コードから団体名を返す。無効・停止中は黙って無視せず、理由つきで知らせる。"""
    try:
    ...
    if not code:
        return {"ok": False, "reason": "empty", "message": "団体コードを入力してください"}
